...
const moment = require('moment-mini'); // Make sure Moment serialization preserves utcOffset. See https://momentjs.com/docs/#/displaying/as-json/
...
class TemplateInstance {
  ...
  parse(input, currentTime, fileName) {
    let text = TemplateLoader.normalizeText(input); // Roundtrip the sample through the Commonmark parser

    text = this.getTemplate().getParserManager().roundtripMarkdown(text); // Set the current time and UTC Offset
    ...
    if (parser.results.length !== 1) {
      ...
      parser.results.forEach(function (element) {
        if (head !== JSON.stringify(element)) {
          const err = "Ambiguous text. Got ".concat(parser.results.length, " ASTs for text: ").concat(text);
  ...
  static convertFormattedParsed(obj, utcOffset) {
    if (obj.$class === 'ParsedDateTime') {
      ...
      if (!instance) {
        throw new Error("Failed to handle datetime ".concat(JSON.stringify(obj, null, 4)));
      }
      ...
      if (obj.currencyCode && obj.currencySymbol && obj.currencyCode !== obj.currencySymbol) {
        throw new Error("Currency symbol ".concat(obj.currencySymbol, " and currency code ").concat(obj.currencyCode, " are incompatible"));
      }
      ...
      Object.entries(obj).forEach((_ref) => {
        let [key, value] = _ref;
        obj[key] = TemplateInstance.convertFormattedParsed(value, utcOffset);
      });
    }
  ...
  draft(options, currentTime) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const logicManager = _this.getLogicManager();

      const clauseId = _this.getIdentifier();

      const contract = _this.getData();

      return logicManager.compileLogic(false).then(
      /*#__PURE__*/
      _asyncToGenerator(function* () {
        const result = yield _this.getEngine().draft(logicManager, clauseId, contract, {}, currentTime, markdownOptions); // Roundtrip the response through the Commonmark parser

        return _this.getTemplate().getParserManager().formatText(result.response, options);
      }));
    })();
  }
  /**
