...
class GrammarVisitor {
  ...
  visitModelManager(modelManager, parameters) {
    debug('entering visitModelManager'); // Save the model manager so that we have access to it later.

    parameters.modelManager = modelManager; // Visit all of the files in the model manager.
  ...
  visitModelFile(modelFile, parameters) {
    debug('entering visitModelFile', modelFile.getNamespace()); // Save the model file so that we have access to it later.

    parameters.modelFile = modelFile; // Visit all of class declarations, but ignore the abstract ones and system ones.
  ...
  visitClassDeclaration(classDeclaration, parameters) {
    debug('entering visitClassDeclaration', classDeclaration.getName()); // do not visit the template model itself, as we need to generate
    // that from the template grammar, including all the source text.

    if (!classDeclaration.getDecorator('AccordTemplateModel') && // TODO (MCR) Why is the following line needed?
    // Ignore classes that have no fields
    classDeclaration.getProperties().length > 0) {
      let result = []; // Walk over all of the properties of this class and its super classes.
      ...
      classDeclaration.getProperties().forEach((property, index) => {
        ...
        properties.push("".concat(property.getName(), " : data[").concat(index * 2, "]").concat(sep));
      });
      parameters.rules.push({
  ...
  visitEnumValueDeclaration(enumValueDeclaration, parameters) {
    ...
    return "\"".concat(enumValueDeclaration.getName(), "\" {% id %}");
  }
  /**
   * Visitor design pattern
