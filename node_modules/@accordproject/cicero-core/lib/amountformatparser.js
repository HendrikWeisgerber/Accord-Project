...
class AmountFormatParser extends FormatParser {
  ...
  buildFormatRules(formatString) {
    ...
    let rules = [];
    let fields = input.split(/(0.0.00?0?)/); // remove the first and empty items
    ...
    fields.forEach((field, index) => {
      ...
      if (fieldName) {
        if (fieldNames.indexOf(fieldName) >= 0) {
          ...
          parsedAmount += "   \"".concat(fieldName, "\": d[").concat(index, "]");
    ...
    const action1 = "{% (d) => {return ".concat(parsedAmount, ";}%}");
    rules.push({
      tokens: tokens1,
      name: name1,
      action: action1
