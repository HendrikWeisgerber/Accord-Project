  ...
  deserialize(el) {
    ...
    if (el.nodeType === 3 && !isIgnorable(el)) {
      ...
      const textNodes = textArray.map(text => {
        if (text) {
          return {
            '$class': "".concat(NS_PREFIX_CommonMarkModel, 'Text'),
            text
          };
        }
      });
      const result = [...textNodes].map((node, i) => i < textNodes.length - 1 ? [node, {
        '$class': "".concat(NS_PREFIX_CommonMarkModel, 'Softbreak')
      }] : [node]).reduce((a, b) => a.concat(b)).filter(n => !!n);
      return result;
    }
  }
...
const LIST_RULE = {
  deserialize(el, next) {
    if (el.tagName && el.tagName.toLowerCase() === 'ul') {
      return {
        '$class': "".concat(NS_PREFIX_CommonMarkModel, "List"),
        type: 'bullet',
    ...
    if (el.tagName && el.tagName.toLowerCase() === 'li') {
      return {
        '$class': "".concat(NS_PREFIX_CommonMarkModel, "Item"),
        nodes: next(el.childNodes)
      };
    }
  }
...
const PARAGRAPH_RULE = {
  deserialize(el, next) {
    if (el.tagName && el.tagName.toLowerCase() === 'p') {
      return {
        '$class': "".concat(NS_PREFIX_CommonMarkModel, "Paragraph"),
        nodes: next(el.childNodes)
      };
    }
  }
...
const STRONG_RULE = {
  deserialize(el, next) {
    if (el.tagName && el.tagName.toLowerCase() === 'strong') {
      return {
        '$class': "".concat(NS_PREFIX_CommonMarkModel, "Strong"),
        nodes: next(el.childNodes)
      };
    }
  }
...
const EMPH_RULE = {
  deserialize(el, next) {
    if (el.tagName && el.tagName.toLowerCase() === 'em') {
      return {
        '$class': "".concat(NS_PREFIX_CommonMarkModel, "Emph"),
        nodes: next(el.childNodes)
      };
    }
  }
...
const LINK_RULE = {
  deserialize(el, next) {
    if (el.tagName && el.tagName.toLowerCase() === 'a') {
      return {
        '$class': "".concat(NS_PREFIX_CommonMarkModel, "Link"),
        nodes: next(el.childNodes),
...
const HEADING_RULE = {
  deserialize(el, next) {
    if (el.tagName) {
      ...
      if (level) {
        return {
          '$class': "".concat(NS_PREFIX_CommonMarkModel, "Heading"),
          nodes: next(el.childNodes),
          level
        };
      }
...
const THEMATIC_BREAK_RULE = {
  deserialize(el, next) {
    if (el.tagName && el.tagName.toLowerCase() === 'hr') {
      return {
        '$class': "".concat(NS_PREFIX_CommonMarkModel, "ThematicBreak")
      };
    }
  }
    ...
    if (el.tagName && el.tagName.toLowerCase() === 'pre' && el.getAttribute('class') === 'code_block') {
      ...
      if (children.length === 1 && children[0].tagName.toLowerCase() === 'code') {
        ...
        if (info) {
          ...
          return {
            '$class': "".concat(NS_PREFIX_CommonMarkModel, "CodeBlock"),
...
const INLINE_CODE_RULE = {
  deserialize(el, next) {
    if (el.tagName && el.tagName.toLowerCase() === 'code') {
      {
        return {
          '$class': "".concat(NS_PREFIX_CommonMarkModel, "Code"),
          text: el.textContent
        };
      }
    }
...
const BLOCK_QUOTE_RULE = {
  deserialize(el, next) {
    if (el.tagName && el.tagName.toLowerCase() === 'blockquote') {
      return {
        '$class': "".concat(NS_PREFIX_CommonMarkModel, "BlockQuote"),
        nodes: next(el.childNodes)
      };
    }
  }
...
const CLAUSE_RULE = {
  deserialize(el, next) {
    ...
    if (tag && tag.toLowerCase() === 'div' && el.getAttribute('class') === 'clause') {
      return {
        '$class': "".concat(NS_PREFIX_CiceroMarkModel, "Clause"),
  ...
  deserialize(el, next) {
    ...
    if (tagName && tagName.toLowerCase() === 'span' && el.getAttribute('class') === 'variable') {
      if (el.getAttribute('format')) {
        return {
          '$class': "".concat(NS_PREFIX_CiceroMarkModel, "Variable"),
...
const CONDITIONAL_VARIABLE_RULE = {
  deserialize(el, next) {
    ...
    if (tagName && tagName.toLowerCase() === 'span' && el.getAttribute('class') === 'conditional') {
      return {
        '$class': "".concat(NS_PREFIX_CiceroMarkModel, "ConditionalVariable"),
        id: el.getAttribute('id'),
...
const COMPUTED_VARIABLE_RULE = {
  deserialize(el, next) {
    ...
    if (tagName && tagName.toLowerCase() === 'span' && el.getAttribute('class') === 'computed') {
      return {
        '$class': "".concat(NS_PREFIX_CiceroMarkModel, "ComputedVariable"),
        value: el.textContent
      };
    }
  }
    ...
    if (tagName && tagName.toLowerCase() === 'span' && el.getAttribute('class') === 'html_inline') {
      {
        ...
        return {
          '$class': "".concat(NS_PREFIX_CommonMarkModel, "HtmlInline"),
          text: text,
          tag
        };
      }
  ...
  deserialize(el, next) {
    if (el.tagName && el.tagName.toLowerCase() === 'pre' && el.getAttribute('class') === 'html_block') {
      ...
      if (children.length === 1 && children[0].tagName.toLowerCase() === 'code') {
        ...
        const tag = CommonMarkTransformer.parseHtmlBlock(text);
        return {
          '$class': "".concat(NS_PREFIX_CommonMarkModel, "HtmlBlock"),
          text: text,
          tag
        };
      }
...
const rules = [LIST_RULE, PARAGRAPH_RULE, STRONG_RULE, EMPH_RULE, LINK_RULE, HEADING_RULE, THEMATIC_BREAK_RULE, CODE_BLOCK_RULE, INLINE_CODE_RULE, BLOCK_QUOTE_RULE, CLAUSE_RULE, VARIABLE_RULE, CONDITIONAL_VARIABLE_RULE, SOFTBREAK_RULE, COMPUTED_VARIABLE_RULE, TEXT_RULE, HTML_INLINE_RULE, HTML_BLOCK_RULE];
