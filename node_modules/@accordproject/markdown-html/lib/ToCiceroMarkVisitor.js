...
class ToCiceroMarkVisitor {
  ...
  deserializeElement(element) {
    let node; //console.log('tagName', element.tagName);
    ...
    for (const rule of this.rules) {
      ...
      if (ret === undefined) {
        ...
        return null; // } else if (ret.object === 'mark') {
  ...
  deserializeElements() {
    ...
    elements.filter(this.cruftNewline).forEach(element => {
      ...
      const node = this.deserializeElement(element); // console.log('node -- ', node);
  ...
  toCiceroMark(input) {
    ...
    let fragment; // eslint-disable-next-line no-undef
    ...
    const children = Array.from(fragment.childNodes); // console.log('children -- ', children);

    const nodes = this.deserializeElements(children); // console.log('nodes', nodes);

    return {
      '$class': "".concat(NS_PREFIX_CommonMarkModel, 'Document'),
      nodes,
