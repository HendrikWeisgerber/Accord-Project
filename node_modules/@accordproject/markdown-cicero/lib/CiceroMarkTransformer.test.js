...
const CiceroMarkTransformer = require('./CiceroMarkTransformer'); // eslint-disable-next-line no-unused-vars
...
expect.extend({
  toMarkdownRoundtrip(markdownText) {
    ...
    const message = pass ? () => this.utils.matcherHint("toMarkdownRoundtrip - ".concat(markdownText, " -> ").concat(newMarkdown), undefined, undefined, undefined) + '\n\n' + "Expected: ".concat(this.utils.printExpected(json1), "\n") + "Received: ".concat(this.utils.printReceived(json2)) : () => {
      const diffString = diff(json1, json2, {
        expand: true
      });
      return this.utils.matcherHint("toMarkdownRoundtrip - ".concat(JSON.stringify(markdownText), " -> ").concat(JSON.stringify(newMarkdown)), undefined, undefined, undefined) + '\n\n' + (diffString && diffString.includes('- Expect') ? "Difference:\n\n".concat(diffString) : "Expected: ".concat(this.utils.printExpected(json1), "\n") + "Received: ".concat(this.utils.printReceived(json2)));
    };
    return {
      actual: markdownText,
      message,
...
function getMarkdownSpecFiles() {
  ...
  specExamples.forEach(function (example) {
    result.push(["".concat(example.section, "-").concat(example.number), example.markdown]);
  });
  return result;
}
/**
...
describe('markdown', () => {
  getMarkdownFiles().forEach((_ref) => {
    let [file, markdownText] = _ref;
    it("converts ".concat(file, " to concerto"), () => {
      const json = ciceroMarkTransformer.fromMarkdown(markdownText, 'json');
      expect(json).toMatchSnapshot();
    });
    it("roundtrips ".concat(file), () => {
...
describe('acceptance', () => {
  it('converts acceptance to CommonMark DOM', () => {
    ...
    const json = ciceroMarkTransformer.fromMarkdown(markdownText, 'json'); // console.log(JSON.stringify(json, null, 4));
  ...
  it('converts acceptance clause content to CommonMark string (no wrapVariable)', () => {
    ...
    expect(json).toMatchSnapshot();
    const newMarkdown = ciceroMarkTransformer.toCommonMark(json.nodes[2], 'json', {
      wrapVariables: false
    }).text;
    expect(newMarkdown).toMatchSnapshot();
    ...
    expect(ciceroMarkTransformer.getClauseText.bind(json.nodes[1], {
      wrapVariables: false
    })).toThrow('Cannot apply getClauseText to non-clause node');
  });
...
describe('markdown-spec', () => {
  getMarkdownSpecFiles().forEach((_ref2) => {
    let [file, markdownText] = _ref2;
    it("converts ".concat(file, " to concerto"), () => {
      const json = ciceroMarkTransformer.fromMarkdown(markdownText, 'json');
      expect(json).toMatchSnapshot();
    ...
    it.skip("roundtrips ".concat(file), () => {
      expect(markdownText).toMarkdownRoundtrip();
    });
  });
});