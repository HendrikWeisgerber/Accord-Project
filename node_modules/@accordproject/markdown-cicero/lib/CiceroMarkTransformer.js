...
class CiceroMarkTransformer {
  ...
  fromCommonMark(input) {
    ...
    let result; // remove variables, e.g. {{ variable }}, {{% computed variable %}}
  ...
  toCommonMark(input) {
    ...
    if (options && options.removeFormatting) {
      ...
      json = cmt.removeFormatting(json); // now we need to also remove the formatting for clause nodes

      json.nodes.filter(element => element.$class === 'org.accordproject.ciceromark.Clause').forEach(clause => {
        const unformatted = cmt.removeFormatting(Object.assign({}, json, {
          nodes: clause.nodes
        }));
    ...
    const concertoDom = this.serializer.fromJSON(json); // convert to common mark
