...
} = require('./externalModels/CiceroMarkModel');
/**
...
class ToCiceroMarkVisitor {
  ...
  visit(thing, parameters) {
    switch (thing.getType()) {
      case 'CodeBlock':
        {
          ...
          if (tag && tag.tagName === 'clause' && tag.attributes.length === 2) {
            ...
            if (ToCiceroMarkVisitor.getAttribute(tag.attributes, 'src') && ToCiceroMarkVisitor.getAttribute(tag.attributes, 'clauseid')) {
            ...
            const ciceroMarkTag = NS_PREFIX_CiceroMarkModel + 'ListVariable'; // Remove last new line, needed by CommonMark parser to identify ending code block (\n```)
            ...
            if (newNodes.length === 1 && newNodes[0].getType() === 'List') {
              ...
              delete thing.tag;
              delete thing.info;
            } else {//console.log('List block does not contain a list');
            }
          }
        }
        break;
      ...
      case 'HtmlInline':
        {
          if (thing.tag && thing.tag.tagName === 'variable' && (thing.tag.attributes.length === 2 || thing.tag.attributes.length === 3)) {
            const tag = thing.tag;

            if (ToCiceroMarkVisitor.getAttribute(tag.attributes, 'id') && ToCiceroMarkVisitor.getAttribute(tag.attributes, 'value')) {
              ...
              delete thing.tag;
              delete thing.text;
            } else {//console.log('Found Variable but without \'id\' and \'value\' attributes ');
            }
          }

          if (thing.tag && thing.tag.tagName === 'if' && thing.tag.attributes.length === 4) {
            const tag = thing.tag;

            if (ToCiceroMarkVisitor.getAttribute(tag.attributes, 'id') && ToCiceroMarkVisitor.getAttribute(tag.attributes, 'value') && ToCiceroMarkVisitor.getAttribute(tag.attributes, 'whenTrue') && ToCiceroMarkVisitor.getAttribute(tag.attributes, 'whenFalse')) {
          ...
          if (thing.tag && thing.tag.tagName === 'computed' && thing.tag.attributes.length === 1) {
            ...
            if (ToCiceroMarkVisitor.getAttribute(tag.attributes, 'value')) {
              ...
              delete thing.tag;
              delete thing.text;
            } else {//console.log('Found ComputedVariable but without \'value\' attributes ');
            }
          }
        }
        break;
