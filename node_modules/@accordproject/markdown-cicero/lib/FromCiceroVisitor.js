      ...
      case 'Clause':
        {
          let jsonSource = {};
          let jsonTarget = {};
          FromCiceroVisitor.visitChildren(this, thing, parameters); // Revert to CodeBlock

          jsonTarget.$class = NS_PREFIX_CommonMarkModel + 'CodeBlock'; // Get the content
          ...
          const attributeString = "src=\"".concat(clauseJson.src, "\" clauseid=\"").concat(clauseJson.clauseid, "\"");
          ...
          thing.info = "<clause ".concat(attributeString, "/>");
        }
        break;
      ...
      case 'ComputedVariable':
        {
          ...
          thing.$classDeclaration = parameters.modelManager.getType(NS_PREFIX_CommonMarkModel + 'HtmlInline'); // Create the text for that document

          const content = '';
          const formatString = thing.format ? " format=\"".concat(encodeURIComponent(thing.format), "\"") : '';
          const attributeString = thingType === 'ComputedVariable' ? "value=\"".concat(encodeURIComponent(thing.value), "\"").concat(formatString) : "id=\"".concat(thing.id, "\" value=\"").concat(encodeURIComponent(thing.value), "\"").concat(formatString);
          const tagName = thingType === 'ComputedVariable' ? 'computed' : 'variable';

          if (this.options && !this.options.wrapVariables) {
            thing.text = thingType === 'ComputedVariable' ? "{{".concat(thing.value, "}}") : thing.value;
          } else {
            thing.text = "<".concat(tagName, " ").concat(attributeString, "/>");
          } // Create the proper tag
      ...
      case 'ConditionalVariable':
        {
          ...
          const content = '';
          const attributeString = "id=\"".concat(thing.id, "\" value=\"").concat(encodeURIComponent(thing.value), "\" whenTrue=\"").concat(encodeURIComponent(thing.whenTrue), "\" whenFalse=\"").concat(encodeURIComponent(thing.whenFalse), "\"");
          const tagName = 'if';
